  ...
  try {
    ...
    if (legacy.raw !== null) {
      ...
      try {
        ...
        // harmless and will be retried on the next launch. persistNow already
        // schedules another write, so do not merge native counters a second
        // time by restarting initialization.
        console.warn("Could not migrate legacy desktop stats", error);
      }
    } else if (dirty) {
      scheduleSave();
    }
